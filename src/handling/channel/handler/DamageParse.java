        ...
        if ((totDamage > 1) && (GameConstants.getAttackDelay(attack.skill, theSkill) >= 100)) {
            ...
            if (tracker.getAttacksWithoutHit() > 1000) {
                tracker.registerOffense(CheatingOffense.ATTACK_WITHOUT_GETTING_HIT, Integer.toString(tracker.getAttacksWithoutHit()));
                        ...
                        if (player.getSkillLevel(4100011) > 0) { //마크 오브 어쌔신
...
//            }
                       ...
                       //MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, eff.getSkillStats().getStats("dot")), SkillFactory.getSkill(4100011), null, false);
                        //monsterStatusEffect.setOwnerId(player.getId()); //cid가 맞아야 보이므로
                      //  monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, eff.getourceId().getStats("dot")), SkillFactory.getSkill(4100011), null, false), true, eff.getDuration(), false);
